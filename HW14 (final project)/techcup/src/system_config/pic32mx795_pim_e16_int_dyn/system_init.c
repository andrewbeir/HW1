...
void setup() {
    ...
    TRISBbits.TRISB5  = 0; // output enable 1 (PWM)
    TRISBbits.TRISB7  = 0; // output phase  1 (0 --> fwd, 1 --> rev)
    TRISBbits.TRISB14 = 0; // output phase  2 (0 --> fwd, 1 --> rev)
    TRISBbits.TRISB15 = 0; // output enable 2 (PWM)

    LATBCLR = 0x4080;      // set pins 7 and 14 to LOW  (forward)
    // LATBSET = 0x4080;   // set pins 7 and 14 to HIGH (reverse)
